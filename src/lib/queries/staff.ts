...
export async function getAllStaff(params?: {
  ...
  const result = staffRows.map((s: any) => {
    ...
    return {
      ...
      paymentType: (s.payment_type === "PER_DAY" || s.payment_type === "MONTHLY" ? s.payment_type : "PER_DAY") as "PER_DAY" | "MONTHLY",
...
export async function getStaffAvailability(startDate: string, endDate: string, roleFilter?: string) {
  ...
  for (const row of staffRows) {
    ...
    result.push({
      ...
      paymentType: (row.payment_type === "PER_DAY" || row.payment_type === "MONTHLY" ? row.payment_type : "PER_DAY"),
